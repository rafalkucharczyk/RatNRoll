...
b2dJson::b2dJson(bool useHumanReadableFloats) { m_useHumanReadableFloats = useHumanReadableFloats; }

b2dJson::~b2dJson()
{
    for (int i = 0; i < (int)m_images.size(); i++)
        delete m_images[i];
    for (std::map<void *, b2dJsonCustomProperties *>::iterator it = m_customPropertiesMap.begin();
         it != m_customPropertiesMap.end(); ++it)
        delete it->second;
}
...
Json::Value b2dJson::b2j(b2Joint *joint)
{
    ...
    case e_revoluteJoint: {
        ...
        floatToJson("refAngle",
                    bodyB->GetAngle() - bodyA->GetAngle() - revoluteJoint->GetJointAngle(),
                    jointValue);
    ...
    case e_pulleyJoint: {
        ...
        floatToJson("lengthA",
                    (pulleyJoint->GetGroundAnchorA() - pulleyJoint->GetAnchorA()).Length(),
                    jointValue);
        floatToJson("lengthB",
                    (pulleyJoint->GetGroundAnchorB() - pulleyJoint->GetAnchorB()).Length(),
                    jointValue);
    ...
    case e_motorJoint: {
        ...
        // floatToJson("correctionFactor", motorJoint->GetCorrectionFactor(),
        // jointValue);
    } break;
    case e_weldJoint: {
    ...
    case e_unknownJoint:
    default:
        std::cout << "Unknown joint type not stored in snapshot : " << joint->GetType()
                  << std::endl;
    }
...
void b2dJson::setBodyName(b2Body *body, const char *name) { m_bodyToNameMap[body] = name; }
...
void b2dJson::setJointName(b2Joint *joint, const char *name) { m_jointToNameMap[joint] = name; }
...
void b2dJson::addImage(b2dJsonImage *image) { setImageName(image, image->name.c_str()); }
...
{
    return getCustomPropertiesForItem(item, false) != NULL &&
           getCustomPropertiesForItem(item, false)->m_customPropertyMap_float.count(propertyName) >
               0;
}
bool b2dJson::hasCustomString(void *item, string propertyName)
{
    return getCustomPropertiesForItem(item, false) != NULL &&
           getCustomPropertiesForItem(item, false)->m_customPropertyMap_string.count(propertyName) >
               0;
}
bool b2dJson::hasCustomVector(void *item, string propertyName)
{
    return getCustomPropertiesForItem(item, false) != NULL &&
           getCustomPropertiesForItem(item, false)->m_customPropertyMap_b2Vec2.count(propertyName) >
               0;
}
bool b2dJson::hasCustomBool(void *item, string propertyName)
{
    return getCustomPropertiesForItem(item, false) != NULL &&
           getCustomPropertiesForItem(item, false)->m_customPropertyMap_bool.count(propertyName) >
               0;
}
bool b2dJson::hasCustomColor(void *item, string propertyName)
{
    return getCustomPropertiesForItem(item, false) != NULL &&
           getCustomPropertiesForItem(item, false)->m_customPropertyMap_color.count(propertyName) >
               0;
}
...
b2dJsonColor4 b2dJson::getCustomColor(void *item, string propertyName, b2dJsonColor4 defaultVal)
{
    ...
    if (!props)
        return defaultVal;
    std::map<string, b2dJsonColor4>::iterator it =
        props->m_customPropertyMap_color.find(propertyName);
    if (it != props->m_customPropertyMap_color.end())
        return it->second;
    return defaultVal;
}

// this define saves us writing out 20 functions which are almost exactly the
// same
#define IMPLEMENT_GET_BY_CUSTOM_PROPERTY_FUNCTIONS_VECTOR(b2Type, ucName, lcName, ucValType,       \
                                                          lcValType)                               \
    int b2dJson::get##ucName##ByCustom##ucValType(                                                 \
        std::string propertyName, lcValType valueToMatch, std::vector<b2Type *> &items)            \
    {                                                                                              \
        set<b2Type *>::iterator it = m_##lcName##WithCustomProperties.begin();                     \
        set<b2Type *>::iterator end = m_##lcName##WithCustomProperties.end();                      \
        while (it != end) {                                                                        \
            b2Type *item = *it;                                                                    \
            if (hasCustom##ucValType(item, propertyName) &&                                        \
                getCustom##ucValType(item, propertyName) == valueToMatch)                          \
...
// this define saves us writing out 20 functions which are almost exactly the
// same
#define IMPLEMENT_GET_BY_CUSTOM_PROPERTY_FUNCTIONS_SINGLE(b2Type, ucName, lcName, ucValType,       \
                                                          lcValType)                               \
    b2Type *b2dJson::get##ucName##ByCustom##ucValType(std::string propertyName,                    \
                                                      lcValType valueToMatch)                      \
    {                                                                                              \
        set<b2Type *>::iterator it = m_##lcName##WithCustomProperties.begin();                     \
        set<b2Type *>::iterator end = m_##lcName##WithCustomProperties.end();                      \
        while (it != end) {                                                                        \
...
#define FILL_CUSTOM_PROPERTY_JSON_VALUE(theName, theType)                                          \
    for (std::map<string, theType>::iterator it = props->m_customPropertyMap_##theType.begin();    \
         it != props->m_customPropertyMap_##theType.end(); ++it) {                                 \
    ...
    for (std::map<string, float>::iterator it = props->m_customPropertyMap_float.begin();
         it != props->m_customPropertyMap_float.end(); ++it) {
        Json::Value propValue;
        propValue["name"] = it->first;
    ...
    for (std::map<string, b2Vec2>::iterator it = props->m_customPropertyMap_b2Vec2.begin();
         it != props->m_customPropertyMap_b2Vec2.end(); ++it) {
        Json::Value propValue;
        propValue["name"] = it->first;
    ...
    for (std::map<string, b2dJsonColor4>::iterator it = props->m_customPropertyMap_color.begin();
         it != props->m_customPropertyMap_color.end(); ++it) {
        Json::Value propValue;
        propValue["name"] = it->first;
...
b2World *b2dJson::readFromFile(const char *filename, std::string &errorMsg, b2World *existingWorld)
{
    ...
    if (!reader.parse(ifs, worldValue)) {
        errorMsg = string("Failed to parse '") + string(filename) + string("' : ") +
                   reader.getFormatedErrorMessages();
        ifs.close();
        return NULL;
    }
...
b2World *b2dJson::j2b2World(Json::Value &worldValue, b2World *world)
{
    ...
    // if ( ! world->GetAutoClearForces() ) { std::cout << "Note: world is not set
    // to auto clear forces.\n"; recreationMayDiffer = true; }
    // if ( world->GetWarmStarting() ) { std::cout << "Note: world is set to use
    // warm starting.\n"; recreationMayDiffer = true; }
    // if ( world->GetContinuousPhysics() ) { std::cout << "Note: world is set to
    // use continuous physics.\n"; recreationMayDiffer = true; }
    // if ( world->GetSubStepping() ) { std::cout << "Note: world is set to use
    // sub stepping.\n"; recreationMayDiffer = true; }
    // if ( worldValue["hasDestructionListener"].asBool() ) { std::cout << "Note:
    // world originally had a destruction listener set.\n"; recreationMayDiffer =
    // true; }
    // if ( worldValue["hasContactFilter"].asBool() ) { std::cout << "Note: world
    // originally had a contact filter set.\n"; recreationMayDiffer = true; }
    // if ( worldValue["hasContactListener"].asBool() ) { std::cout << "Note:
    // world originally had a contact listener set.\n"; recreationMayDiffer =
    // true; }
...
b2Joint *b2dJson::j2b2Joint(b2World *world, Json::Value &jointValue)
{
    ...
    if (type == "revolute") {
        ...
        jointDef = &motorDef;
        motorDef.linearOffset =
            jsonToVec("anchorA", jointValue); // editor uses anchorA as the linear offset
...
{
    char buf[16];
    // sprintf(buf, "%08X", *((int*)(&f)) ); dereferencing type-punned pointer
...
float b2dJson::hexToFloat(std::string str)
{
    ...
    // return *((float*)bytes); dereferencing type-punned pointer will break
