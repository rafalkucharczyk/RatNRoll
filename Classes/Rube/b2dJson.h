...
class b2dJson
{
  ...
  public:
    ...
    b2World *readFromFile(const char *filename, std::string &errorMsg,
                          b2World *existingWorld = NULL);

    // backward compatibility
    bool readIntoWorldFromValue(b2World *existingWorld, Json::Value &worldValue)
    {
        return readFromValue(worldValue, existingWorld);
    }
    bool readIntoWorldFromString(b2World *existingWorld, std::string str, std::string &errorMsg)
    {
        return readFromString(str, errorMsg, existingWorld);
    }
    bool readIntoWorldFromFile(b2World *existingWorld, const char *filename, std::string &errorMsg)
    {
        return readFromFile(filename, errorMsg, existingWorld);
    }
...
#define DECLARE_SET_CUSTOM_PROPERTY_VALUE_FUNCTIONS(ucType, lcType)                                \
    void setCustom##ucType(b2Body *item, std::string propertyName, lcType val)                     \
    {                                                                                              \
        m_bodiesWithCustomProperties.insert(item);                                                 \
        setCustom##ucType((void *)item, propertyName, val);                                        \
    }                                                                                              \
    void setCustom##ucType(b2Fixture *item, std::string propertyName, lcType val)                  \
    {                                                                                              \
        m_fixturesWithCustomProperties.insert(item);                                               \
        setCustom##ucType((void *)item, propertyName, val);                                        \
    }                                                                                              \
    void setCustom##ucType(b2Joint *item, std::string propertyName, lcType val)                    \
    {                                                                                              \
        m_jointsWithCustomProperties.insert(item);                                                 \
        setCustom##ucType((void *)item, propertyName, val);                                        \
    }                                                                                              \
    void setCustom##ucType(b2dJsonImage *item, std::string propertyName, lcType val)               \
    {                                                                                              \
        m_imagesWithCustomProperties.insert(item);                                                 \
        setCustom##ucType((void *)item, propertyName, val);                                        \
    }                                                                                              \
    void setCustom##ucType(b2World *item, std::string propertyName, lcType val)                    \
    {                                                                                              \
    ...
    b2dJsonColor4 getCustomColor(void *item, std::string propertyName,
                                 b2dJsonColor4 defaultVal = b2dJsonColor4());
...
#define DECLARE_GET_BY_CUSTOM_PROPERTY_VALUE_FUNCTIONS_VECTOR(ucType, lcType)                      \
    int getBodiesByCustom##ucType(std::string propertyName, lcType valueToMatch,                   \
                                  std::vector<b2Body *> &bodies);                                  \
    int getFixturesByCustom##ucType(std::string propertyName, lcType valueToMatch,                 \
                                    std::vector<b2Fixture *> &fixtures);                           \
    int getJointsByCustom##ucType(std::string propertyName, lcType valueToMatch,                   \
                                  std::vector<b2Joint *> &joints);                                 \
    int getImagesByCustom##ucType(std::string propertyName, lcType valueToMatch,                   \
...
#define DECLARE_GET_BY_CUSTOM_PROPERTY_VALUE_FUNCTIONS_SINGLE(ucType, lcType)                      \
  ...
  public:
    ...
    static float jsonToFloat(const char *name, Json::Value &value, int index = -1,
                             float defaultValue = 0);
    static b2Vec2 jsonToVec(const char *name, Json::Value &value, int index = -1,
                            b2Vec2 defaultValue = b2Vec2(0, 0));
};
